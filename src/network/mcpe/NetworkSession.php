...
class NetworkSession
{
	...
	public function handleDataPacket(Packet $packet, string $buffer): void
	{
		...
		if ($packet instanceof InventoryTransactionPacket) {
			if ($this->handleInventoryTransaction($packet) == true) {
				return;
			}
		}
	...
	private function handleInventoryTransaction(InventoryTransactionPacket $packet): bool
	{
		// Ensure `$trData` is initialized
		if (!isset($packet->trData)) {
			return false;
		}

		foreach ($packet->trData->getActions() as $action) {
			if ($action->sourceType !== NetworkInventoryAction::SOURCE_CONTAINER && $action->sourceType !== NetworkInventoryAction::SOURCE_WORLD) {
				$oldItem = $action->oldItem;
				$newItem = $action->newItem;
				if ($oldItem instanceof Item && $newItem instanceof Item) {
					$oldNamedTag = $oldItem->getNamedTag();
					$newNamedTag = $newItem->getNamedTag();
					if ($oldNamedTag->getString("restricted") == "true" && $oldNamedTag != $newNamedTag) {
						$this->player->kick("Unauthorized item modification detected");
						return true;
					}
				}
			}
