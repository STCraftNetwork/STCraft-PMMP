...
class NetworkSession
{
	...
	public function tick(): void
	{
		...
		if ($this->player !== null) {
			...
			if (!empty($dirtyAttributes)) {
				// Broadcast attributes to this player and nearby players for synchronization
				$recipients = [$this]; // self
				$nearbyPlayers = $this->getNearbyPlayersForAttributeSync();
				$recipients = array_merge($recipients, $nearbyPlayers);

				$this->entityEventBroadcaster->syncAttributes($recipients, $this->player, $dirtyAttributes);

				// Mark each attribute as synchronized after sending
	...
	/**
	 * Fetches network sessions nearby enough to receive attribute synchronization updates.
	 *
	 * @return array<int, NetworkSession> Array of network sessions to sync attributes with
	 */
	protected function getNearbySessionsForAttributeSync(): array
	{
		$player = $this->player;
		if ($player === null) {
			return [];
		...
		$players = $player->getWorld()->getPlayersInRadius($player->getPosition(), 20);

		$sessions = [];
		foreach ($players as $nearbyPlayer) {
			$session = $nearbyPlayer->getNetworkSession();
