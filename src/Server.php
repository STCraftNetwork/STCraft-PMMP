...
class Server
{
	...
	{
		$this->getLogger()->warning("Deprecated method getPlayerByPrefix() used. Use getPlayersByPrefix() instead. Removing in future.");
		$found = null;
		$name = strtolower($name);
		$delta = PHP_INT_MAX;
	...
	/**
	 * Returns a list of online players whose names begin with or equal the given string (case insensitive).
	 * Useful for resolving potential matches without ambiguity.
	 *
	 * @param string $name
	 * @return Player[]
	 */
	public function getPlayersByPrefix(string $name): array
	{
		$name = strtolower($name);
		$matches = [];
