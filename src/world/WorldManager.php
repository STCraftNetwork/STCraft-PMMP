...
class WorldManager
{
	...
	public function tick(int $currentTick): void
	{
		...
		$autoSaveTicks = $this->autoSaveTicks;
		$tickTarget = Server::TARGET_SECONDS_PER_TICK * 1000;
		...
		foreach ($worlds as $k => $world) {
			...
			$tickMs = (hrtime(true) - $startNs) / 1e6;
			$world->tickRateTime = $tickMs;

			if ($tickMs >= $tickTarget) {
				$world->getLogger()->debug(
					sprintf("Tick took too long: %.2fms (%.2f ticks)", $tickMs, $tickMs / $tickTarget)
				);
			}
		}

		if ($autoSave && ++$this->autoSaveTicker >= $autoSaveTicks) {
			$this->autoSaveTicker = 0;
			$logger->debug("[Auto Save] Saving worlds...");

			$startNs = hrtime(true);
			$this->doAutoSave();
			$elapsedMs = (hrtime(true) - $startNs) / 1e6;

			$logger->debug(
				"[Auto Save] Save completed in " .
				($elapsedMs >= 1000 ? round($elapsedMs / 1000, 3) . "s" : round($elapsedMs, 3) . "ms")
			);
		}
	}
