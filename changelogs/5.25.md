...
### `pocketmine\chestapi`
- Added new methods to handle chest inventory more efficiently.
	- `public ChestInventory->addItem(Item $item) : void` - adds an item to the chest inventory.
	- `public ChestInventory->removeItem(Item $item) : void` - removes an item from the chest inventory.
	- `public ChestInventory->getItems() : array` - returns an array of items currently in the chest inventory.

### `pocketmine\formapi`
- Introduced new form handling capabilities.
	- `public FormAPI->createForm(string $title, array $content) : Form` - creates a new form with the specified title and content.
	- `public FormAPI->sendForm(Player $player, Form $form) : void` - sends the created form to the specified player.

### `pocketmine\pmform`
- Enhanced form management features.
	- `public PMForm->addField(string $name, string $type, mixed $default = null) : void` - adds a new field to the form.
	- `public PMForm->removeField(string $name) : void` - removes a field from the form.
	- `public PMForm->getFields() : array` - returns an array of fields in the form.
